...
file '/tmp/serverspec/mongo-db-node.json' do
  owner 'root'
  mode '0400'
end

log 'Dumping attributes to /tmp/serverspec/mongo-db-node.json'
...
ruby_block 'dump_node_attributes' do
  block do
    ...
    File.open('/tmp/serverspec/mongo-db-node.json', 'w') { |file| file.write(JSON.pretty_generate(attrs)) }
  end
end
